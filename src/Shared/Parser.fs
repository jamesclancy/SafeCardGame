// From https://github.com/swlaschin/fsharpforfunandprofit.com_code/blob/master/posts/understanding-parser-combinators-3/ParserLibrary.fsx
...
let panycharbut charToMatch =
    // label is just the character
    let label = sprintf "Not a %c" charToMatch
...
let stringReturn str returnValue =
    mapP (fun s -> returnValue) (pstring str)

let skipChar char =
    pchar char

// parse a string till it hits a specific char
let charsTillChar char =
    let label = "charsTillChar"

    manyChars1 (panycharbut char)